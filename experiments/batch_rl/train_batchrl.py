"""
Train a batch-rl agent on offline data. Can either pass env as one of the envs from the `offline_rl`, or as a standard env, 
along with a path to a folder containing saved replay buffer to load. 

E.g. 1, using an offline_rl environment:
...
from envs.offline_rl.get_datasets import DATASET_NAMES
...
config.replay_size = int(1e6) + 1 # cannot add 1M entries into size 1M buffer (TODO: fix this)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--num_envs', default=1, type=int, help='number of envs (defaults to procs - 1)')
