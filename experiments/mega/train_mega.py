...
def main(args):
  ...
  if args.ag_curiosity is not None:
    ...
    elif args.ag_curiosity == 'entropygain':
      config.entropy_gain_predictor = EntropyGainPredictor(batch_size=args.succ_bs, history_length=args.succ_hl, optimize_every=args.succ_oe)
      config.ag_curiosity = EntropyGainGoalCuriosity(max_steps=args.env_max_step, use_qcutoff=use_qcutoff, keep_dg_percent=args.keep_dg_percent)
    elif args.ag_curiosity == 'entropygaincond':
      config.mdn_predictor = MixtureDensityNetworkPredictor(batch_size=args.succ_bs, history_length=args.succ_hl, optimize_every=args.succ_oe, num_ags_in_eps=args.ag_pred_ehl)
      config.ag_curiosity = EntropyGainConditionalGoalCuriosity(max_steps=args.env_max_step, use_qcutoff=use_qcutoff, keep_dg_percent=args.keep_dg_percent)
  ...
  if args.ag_curiosity == 'entropygain':
    config.entropy_gain_model = PytorchModel('entropy_gain_model', lambda: Critic(FCBody(e.goal_dim, args.layers, nn.LayerNorm, make_activ(config.activ)), 1))
  
  if args.ag_curiosity == 'entropygaincond':
    # TODO: Add number of components to args
    pi_body = FCBody(e.goal_dim, args.layers, nn.BatchNorm1d)
    normal_body = FCBody(e.goal_dim, args.layers, nn.BatchNorm1d)
    num_components = 2
    config.mixture_density_network = PytorchModel('mixture_density_network', lambda: MixtureDensityNetwork(pi_body, normal_body, e.goal_dim, num_components))
  ...
  if args.visualize_trained_agent:
    ...
    for epoch in range(int(args.max_steps // args.epoch_len)):
      ...
      agent.save('checkpoint')
        ...
        #print("Loading agent at epoch {}".format(epoch))
        #agent.load('checkpoint')
